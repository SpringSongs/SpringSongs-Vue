	...
	export default {
		...
		methods: {
			...
			loadTrea: function(resolve, parentId) {
				let self = this;
				self.$axios.post("/SpringOrganization/listOrganizationsByParent", qs.stringify({
					"parentId": parentId
				})).then((res) => {
			...
			handleSearch: function() {
				...
				self.$axios.post('/SpringOrganization/ListByPage?page=' + self.searchForm.page + "&size=" + self.searchForm.size,
					self.searchForm, {
						timeout: 1000 * 60 * 0.5
					}).then(function(res) {
			...
			handleSave: function(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/SpringOrganization/Create', this.addForm, {
							timeout: 1000 * 60 * 2
						}).then((res) => {
			...
			handleUpdate: function(formName) {
				let self = this;
				this.$axios.post('/SpringOrganization/Edit', this.editForm, {
					timeout: 1000 * 60 * 2
				}).then((res) => {
					self.$message.success(res.data.msg);
					self.handleSearch();
			...
			handleDel: function() {
				...
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					self.$axios.post('/SpringOrganization/SetDeleted', qs.stringify({
						'ids': ids
					}, {
						indices: false
					})).then(function(res) {
			...
			handleSingleDelete(index, row) {
				...
				self.$confirm('确认删除该记录吗?', '提示', {
					...
					axios.post('/SpringOrganization/SetDeleted', qs.stringify(para))
						.then(function(res) {
							self.$message.success(res.data.msg);
							self.handleSearch();
						})
			...
			handleExport: function() {
				axios.post('/SpringOrganization/export')
					.then(function(res) {
						self.$message.success(res.data.msg);
						self.handleSearch();
					})
