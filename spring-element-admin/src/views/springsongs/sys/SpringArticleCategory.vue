				...
				<el-tabs type="border-card">
					<el-tab-pane label="文章分类">
						<div class="treesearch">
							<el-tree lazy node-key="id" :load="handleLoadTrea" :props="ListTreeProps" highlight-current highlight-current
						...
						<el-tab-pane label="文章分类">
							...
							<template>
								<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" highlight-current-row style="width: 100%;"
								 @selection-change="handleSelectionChange">
									...
									<el-table-column prop='keywords' label='关键词' width='180'></el-table-column>
									<el-table-column prop='description' label='描述' width='180'></el-table-column>
									<el-table-column prop='sortOrder' label='排序' width='180'></el-table-column>
							...
							<el-dialog title='新增' :visible.sync='dialogAddVisible' width='50%' :before-close='handleClose'>
								<el-form :model='addForm' label-width='80px' :rules='addFormRules' ref='addForm'>
									...
									<el-form-item label='排序' prop="sortOrder">
										<el-input-number v-model='addForm.sortOrder' auto-complete='off'></el-input-number>
									</el-form-item>
								</el-form>
							...
							<el-dialog title='修改' :visible.sync='dialogEditVisible' width='50%' :before-close='handleClose'>
								<el-form :model='editForm' label-width='80px' :rules='editFormRules' ref='editForm'>
									...
									<el-form-item label='排序' prop="sortOrder">
										<el-input-number v-model='editForm.sortOrder' auto-complete='off'></el-input-number>
									</el-form-item>
								</el-form>
								<span slot='footer' class='dialog-footer'>
	...
	export default {
		...
		methods: {
			...
			handleSearch: function() {
				let self = this;
				if (self.searchForm.currPage == undefined) {
					self.searchForm.currPage = 0;
				}
				self.$axios.post('/SpringArticleCategory/ListByPage?page=' + self.searchForm.page + "&size=" + self.searchForm.size,
					self.searchForm, {
						timeout: 1000 * 60 * 0.5
					}).then(function(res) {
			...
			handleSave: function(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/SpringArticleCategory/Create', this.addForm).then((res) => {
			...
			handleUpdate: function(formName) {
				let self = this;
				this.$axios.post('/SpringArticleCategory/Edit', this.editForm, {
					timeout: 1000 * 60 * 2
				}).then((res) => {
					self.$message.success(res.data.msg);
					self.handleSearch();
			...
			handleDel: function() {
				...
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					self.$axios.post('/SpringArticleCategory/SetDeleted', qs.stringify({
						'ids': ids
					}, {
						indices: false
					})).then(function(res) {
			...
			handleSingleDelete(index, row) {
				...
				self.$confirm('确认删除该记录吗?', '提示', {
					...
					axios.post('/SpringArticleCategory/SetDeleted', qs.stringify(para))
						.then(function(res) {
							self.$message.success(res.data.msg);
							self.handleSearch();
						})
			...
			handleExport: function() {
				axios.post('/SpringArticleCategory/export')
