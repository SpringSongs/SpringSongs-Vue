...
export default {
  ...
  methods: {
    ...
    handleSearch: function() {
      ...
      self.$axios
        .post(
          "/SpringDictionary/ListByPage?page=" +
    ...
    handleSave: function(formName) {
      ...
      self.$refs[formName]
        .validate(valid => {
          if (valid) {
            self.$axios
              .post("/SpringDictionary/Create", this.addForm)
              .then(res => {
    ...
    handleUpdate: function(formName) {
      let self = this;
      self.$axios
        .post("/SpringDictionary/Edit", this.editForm, {
          timeout: 1000 * 60 * 2
        })
        .then(res => {
          self.$message.success(res.data.msg);
    ...
    handleSingleDelete(index, row) {
      ...
      self
        ...
        .then(() => {
          ...
          self.$axios
            .post("/SpringDictionary/SetDeleted", qs.stringify(para))
            .then((res)=> {
              self.$message.success(res.data.msg);
              self.handleSearch();
            })
