						...
						<el-tab-pane label="角色管理">
								 ...
								 @selection-change="handleSelectionChange">
									...
									<el-table-column fixed='right' label='操作' width='400'>
										<template slot-scope='scope'>
											...
											<el-button icon="el-icon-edit" @click='handleSetAuthority(scope.$index, scope.row)' type='text' size='small'>授权</el-button>
							...
							<el-dialog title='新增' :visible.sync='dialogAddVisible' width='50%' :before-close='handleClose'>
								<el-form :model='addForm' label-width='80px' :rules='addFormRules' ref='addForm'>
									...
									<el-form-item label='允许编辑' prop="enableEdit">
										<el-switch v-model="addForm.enableEdit" active-color="#13ce66" inactive-color="#ff4949" :active-value="true"
										 :inactive-value="false">
										</el-switch>
									</el-form-item>
									<el-form-item label='允许删除' prop="enableDelete">
										<el-switch v-model="addForm.enableDelete" active-color="#13ce66" inactive-color="#ff4949" :active-value="true"
										 :inactive-value="false">
										</el-switch>
									</el-form-item>
								</el-form>
							...
							<el-dialog title='修改' :visible.sync='dialogEditVisible' width='50%' :before-close='handleClose'>
								<el-form :model='editForm' label-width='80px' :rules='editFormRules' ref='editForm'>
									...
									<el-form-item label='允许编辑' prop="enableEdit">
										<el-switch v-model="editForm.enableEdit" active-color="#13ce66" inactive-color="#ff4949" :active-value="true"
										 :inactive-value="false">
										</el-switch>
									</el-form-item>
									<el-form-item label='允许删除' prop="enableDelete">
										<el-switch v-model="editForm.enableDelete" active-color="#13ce66" inactive-color="#ff4949" :active-value="true"
										 :inactive-value="false">
										</el-switch>
									</el-form-item>
								</el-form>
							...
							<el-dialog title='用户' :visible.sync='dialogRoleUsersVisible' width='50%' :before-close='handleClose'>
								<el-table ref="multipleTable" :data="tableRoleUserData" tooltip-effect="dark" highlight-current-row style="width: 100%;"
	...
	export default {
		...
		methods: {
			...
			listMenuTree: function() {
				let self = this;
				self.url = "/SpringResource/ListByPage?page=0&size=1000";
				return self.$axios.post(self.url, {});
			},
			listAuthority: function() {
				let self = this;
				return self.$axios.post('/SpringRole/ListAuthority/' + this.roleId);
			},
			...
			handleAuthoritySave: function() {
				...
				self.menuIdList = [].concat(self.$refs.menuListTree.getCheckedKeys(), [this.tempKey], self.$refs.menuListTree
					.getHalfCheckedKeys());
				self.$axios.post('/SpringRole/SetAuthority/' + this.roleId, qs.stringify({
			...
			handleUserSearch: function() {
				...
				if (self.searchUserForm.page == undefined) {
					self.searchUserForm.page = 0;
				}
				self.$axios.post('/SpringUser/ListByPage?page=' + self.searchUserForm.page + "&limit=" + self.searchUserForm.limits,
						self.searchForm)
			...
			handleUserRoleSearch: function() {
				...
				if (self.searchUserRoleForm.page == undefined) {
					self.searchUserRoleForm.page = 0;
				}
				self.$axios.post('/SpringUser/ListByRoleId/' + this.roleId + '?page=' + self.searchUserRoleForm.page +
			...
			handleSearch: function() {
				...
				self.$axios.post('/SpringRole/ListByPage?page=' + self.searchForm.page + "&size=" + self.searchForm.size,
					self.searchForm, {
						timeout: 1000 * 60 * 0.5
					}).then(function(response) {
			...
			handleSave: function(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/SpringRole/Create', this.addForm).then((res) => {
			...
			handleSetUsersSave: function() {
				...
				if (self.multipleSelectionUser.length == 0) {
					...
					this.$axios.post('/SpringRole/SetUsers/' + this.roleId, qs.stringify({
						'ids': ids
					}, {
						indices: false
					})).then((res) => {
			...
			handleUpdate: function(formName) {
				let self = this;
				this.$axios.post('/SpringRole/Edit', this.editForm).then((res) => {
					self.$message.success(res.data.msg);
					self.handleSearch();
			...
			handleDel: function() {
				...
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					self.$axios.post('/SpringRole/SetDeleted', qs.stringify({
						'ids': ids
					}, {
						indices: false
					})).then(function(response) {
			...
			handleSingleDelete(index, row) {
				...
				self.$confirm('确认删除该记录吗?', '提示', {
					...
					axios.post('/SpringRole/SetDeleted', qs.stringify(para))
						.then(function(response) {
							self.$message.success(response.data.msg);
							self.handleSearch();
						})
