...
export default {
	...
	methods: {
		...
		handleSearch: function() {
			...
			self.$axios.post('/SpringJobGroup/ListByPage?page=' + self.searchForm.page + "&size=" + self.searchForm.size, self.searchForm, {
				timeout: 1000 * 60 * 0.5
		...
		handleSave: function(formName) {
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$axios.post('/SpringJobGroup/Create', this.addForm, {
						timeout: 1000 * 60 * 2
					}).then((res) => {
		...
		handleUpdate: function(formName) {
			let self = this;
			this.$axios.post('/SpringJobGroup/Edit', this.editForm, {
				timeout: 1000 * 60 * 2
			}).then((res) => {
				self.$message.success(res.data.msg);
				self.handleSearch();
		...
		handleDel: function() {
			...
			this.$confirm('确认删除该记录吗?', '提示', {
				...
				self.$axios.post('/SpringJobGroup/SetDeleted', qs.stringify({
					'ids': ids
				}, {
					indices: false
				})).then(function(response) {
		...
		handleSingleDelete(index, row) {
			...
			self.$confirm('确认删除该记录吗?', '提示', {
				...
				axios.post('/SpringJobGroup/SetDeleted', qs.stringify(para), )
					.then(function(response) {
						self.$message.success(response.data.msg);
						self.handleSearch();
					})
